...
app.post('/api/product-search', async (req, res) => {
  try {
    const { message, product, productName, ean } = req.body || {};
    const query = String(ean || message || product || productName || '').trim();
...
app.post('/api/product-ai', async (req, res) => {
  try {
    ...
    return res.json({ ok: true, found: true, product: p, answer:
      `Ho trovato il prodotto nel catalogo:\n\n` +
      `📦 ${p.name}\n🏷️ Marca: ${p.brand || 'Non disponibile'}\n🔢 EAN: ${p.ean || 'Non disponibile'}\n💶 Prezzo: ${p.price || 'Non disponibile'}\n\n` +
      `Per non inventare dati tecnici, usa questi riferimenti per la scheda ufficiale: marca "${p.brand || ''}", nome "${p.name || ''}", EAN "${p.ean || ''}". La ricerca tecnica ufficiale avanzata sarà riattivata aggiungendo OpenAI/web search al backend.` });
  } catch (err) {
