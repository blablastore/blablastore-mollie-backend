...
function answerForProduct(row) {
  ...
  return `Ho trovato questo prodotto nel catalogo Blabla Store:\n\n` +
    ...
    `Vuoi che cerchi anche la scheda tecnica ufficiale?`;
}

function isOpenAiConfigured() {
  return Boolean(OPENAI_API_KEY && String(OPENAI_API_KEY).startsWith('sk-'));
}

function buildTechnicalPrompt(product, userQuestion) {
  return `Sei l'assistente prodotti di Blabla Store.

Obiettivo: cercare e riassumere la scheda tecnica ufficiale del prodotto usando soprattutto EAN, marca e modello.

Prodotto catalogo Blabla Store:
- Marca: ${product.brand || 'non disponibile'}
- Nome: ${product.name || 'non disponibile'}
- EAN: ${product.ean || 'non disponibile'}
- Prezzo Blabla Store: ${product.price || 'non disponibile'}
- Categoria: ${product.category || 'non disponibile'}

Domanda cliente: ${userQuestion}

Regole obbligatorie:
1. Dai priorità al sito ufficiale del produttore o a fonti tecniche affidabili.
2. Usa l'EAN per riconoscere il prodotto corretto.
3. Non inventare caratteristiche tecniche.
4. Se non trovi dati affidabili, scrivi chiaramente: "Scheda tecnica ufficiale non trovata".
5. Rispondi in italiano, in modo utile per un cliente e-commerce.
6. Includi sempre prezzo Blabla Store, EAN e una sezione "Fonte".
7. Se trovi dati tecnici, riassumili per punti: display/dimensioni, memoria, connettività, batteria/alimentazione, caratteristiche principali, compatibilità, contenuto confezione se disponibile.`;
}

async function askOpenAIForTechnicalSheet(product, userQuestion) {
  if (!isOpenAiConfigured()) {
    return `Ho trovato il prodotto nel catalogo:\n\n` +
      `📦 ${product.name}\n🏷️ Marca: ${product.brand || 'Non disponibile'}\n🔢 EAN: ${product.ean || 'Non disponibile'}\n💶 Prezzo: ${product.price || 'Non disponibile'}\n\n` +
      `Scheda tecnica ufficiale non trovata: OPENAI_API_KEY non configurata correttamente su Render.`;
  }

  const client = new OpenAI({ apiKey: OPENAI_API_KEY });
  const prompt = buildTechnicalPrompt(product, userQuestion);
  ...
  try {
    ...
    return response.output_text || 'Scheda tecnica ufficiale non trovata.';
  } catch (err) {
    console.error('Errore OpenAI web_search:', err && (err.message || err));

    try {
      const response = await client.responses.create({
        model: OPENAI_MODEL,
        input: prompt + '\n\nNota: se non puoi verificare sul web, rispondi solo con i dati sicuri del catalogo e dichiara che la scheda tecnica ufficiale non è stata trovata.'
      });
      return response.output_text || 'Scheda tecnica ufficiale non trovata.';
    } catch (err2) {
      console.error('Errore OpenAI fallback:', err2 && (err2.message || err2));
      return `Ho trovato il prodotto nel catalogo:\n\n` +
        `📦 ${product.name}\n🏷️ Marca: ${product.brand || 'Non disponibile'}\n🔢 EAN: ${product.ean || 'Non disponibile'}\n💶 Prezzo: ${product.price || 'Non disponibile'}\n\n` +
        `Scheda tecnica ufficiale non trovata. Errore AI: ${err2.message || 'servizio non disponibile'}`;
    }
  }
}
...
app.post('/api/product-ai', async (req, res) => {
  try {
    const { message, query, ean } = req.body || {};
    const q = String(message || query || ean || '').trim();
    if (!q) return res.status(400).json({ ok: false, error: 'Scrivi nome prodotto o EAN.' });

    const rows = await loadProductCatalog();
    const cleaned = q.replace(/scheda tecnica|scheda|caratteristiche|specifiche|ufficiale|datasheet|manuale/gi, ' ').trim() || q;
    const matches = findProducts(rows, cleaned, ean);
    if (!matches.length) {
      return res.json({ ok: true, found: false, answer: 'Non ho trovato il prodotto nel catalogo. Per cercare la scheda tecnica mi serve nome esatto o EAN.' });
    }

    const p = formatProduct(matches[0].row);
    const answer = await askOpenAIForTechnicalSheet(p, q);
    return res.json({ ok: true, found: true, product: p, answer });
  } catch (err) {
    console.error('Errore /api/product-ai:', err);
    res.status(500).json({ ok: false, error: err.message || 'Errore AI prodotto' });
  }
});
